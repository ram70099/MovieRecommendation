...
const Search = () => {
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      const movies: Movie[] = data.map((item: any) => ({
        id: item.MovieID,
        title: item.Title,
        year: item.Year || (item['Release Date'] ? new Date(item['Release Date']).getFullYear() : undefined),
  ...
  return (
    <div className="page">
      ...
      {hasSearched && !isLoading && searchResults.length === 0 && searchQuery.trim().length >= 2 && (
